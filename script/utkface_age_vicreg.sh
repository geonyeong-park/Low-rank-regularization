...
for s in ${seed[@]}; do
    for ld in ${lambdas[@]}; do
        ...
        CUDA_VISIBLE_DEVICES=$1 python run.py --data UTKFace --bias_attr age --target_attr gender --mode_CL vicReg \
        ...
        CUDA_VISIBLE_DEVICES=$1 python run.py --data UTKFace --bias_attr age --target_attr gender --mode_CL vicReg \
            ...
            --oversample_pth "expr/checkpoint/UTKFace_age_vicReg_lambda_"$ld"_seed_"$s"/wrong_idx.pth"
    done
done

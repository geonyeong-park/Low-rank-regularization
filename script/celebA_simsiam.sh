...
lambdas=(0.0 0.001 0.002 0.0008 0.0006)


for ld in ${lambdas[@]}; do
    echo "lambda_offdiag: $ld"
    CUDA_VISIBLE_DEVICES=$1 python run.py --data celebA --target_attr makeup --bias_attr gender --mode_CL SimSiam \
    ...
    CUDA_VISIBLE_DEVICES=$1 python run.py --data celebA --target_attr makeup --bias_attr gender --mode_CL SimSiam \
        ...
        --oversample_pth "expr/checkpoint/celebA_makeup_SimSiam_lambda_"$ld"_seed_$2/wrong_idx.pth"
done

...
for s in ${seed[@]}; do
    for ld in ${lambdas[@]}; do
        ...
        CUDA_VISIBLE_DEVICES=$1 python run.py --data UTKFace --bias_attr age --target_attr gender --mode_CL SimSiam \
        ...
        CUDA_VISIBLE_DEVICES=$1 python run.py --data UTKFace --bias_attr age --target_attr gender --mode_CL SimSiam \
            ...
            --oversample_pth "expr/checkpoint/UTKFace_age_SimSiam_lambda_"$ld"_seed_"$s"/wrong_idx.pth"
    done
done
